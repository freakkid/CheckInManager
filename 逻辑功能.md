...
获取 course_name（课程名）, credit（学分）, semester（学期）, class_time（上课时间）, position（上课地点）, 学生人数 等课程信息 + 教师姓名(teacher_id,teacher_name)
...
获取修改密码页面：GET  /user/{user_id}/user_password
...
页面 GET /teacher/{teacher_id}/add_course

输入框：

手动或文件导入格式都需要前端做判断，与后台交互都是如下：

POST /api/teacher/{teacher_id}/course
+ request
    + HEADERS
...
DELETE /api/teacher/{teacher_id}/course/{course_id}【待定】
...
DELETE /api/teacher/{teacher_id}/courses 【待定】
