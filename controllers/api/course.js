...
export async function createCourse(ctx) {
  if (ctx.is_manager === 1) {
    const { course_name, credit, semester, class_time, position, user_id } = ctx.body;

    if (!course_name || !credit || !semester
      || !class_time || !position || !user_id) {
...
export async function deleteCourse(ctx) {
  if (ctx.is_manager === 1) {
    ...
    if ((await courseModel.deleteCourse(course_id).affectedRows !== 1)) {
      ...
      await courseMemberModel.deleteCourseMember(course_id);
      sendData(ctx, 201, JSON.stringify({ message: '删除课程成功' }));
    }
  } else {
    sendData(ctx, 401, JSON.stringify({ message: '您没有权限' }));
  }
...
export async function deleteCourseMember(ctx) {
  if (ctx.is_manager === 1) {
    ...
    if ((await courseMemberModel.deleteCourseMember(course_id, student_id)).affectedRows !== 1) {
