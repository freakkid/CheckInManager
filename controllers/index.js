import * as userCtrl from './user';
import * as courseCtrl from './course';
import * as studentCtrl from './student';
import * as addStudentCtrl from './add_student';
import * as checkinByQRCodeCtrl from './checkinByQRCode';
import * as apiUserCtrl from './api/user';
import * as apiUsersCtrl from './api/users';
import * as apiCourseCtrl from './course';
import * as apiStudentCtrl from './api/student';
import * as apiStudentsCtrl from './api/students';
import * as apiCheckinStudentCtrl from './api/checkin_student';

export {
  userCtrl, courseCtrl, studentCtrl, addStudentCtrl, checkinByQRCodeCtrl,
  apiUserCtrl, apiUsersCtrl, apiCourseCtrl, apiStudentCtrl, apiStudentsCtrl, apiCheckinStudentCtrl
};
